...
class _MacroScreenState extends State<MacroScreen> {
  final _controller = const MacroController();
  LensRepository _lensRepository = SqliteLensRepository();
  ...
  void didChangeDependencies() {
    ...
    _lensRepository =
        AppDependenciesScope.maybeOf(context)?.lenses ?? SqliteLensRepository();
    _loadLenses();
  }
  ...
  List<MountPreset> get _availableMounts {
    return _controller.resolveAvailableMounts(widget.settings.enabledMountIds);
  }

  MountPreset? get _selectedMount {
    return _controller.findMountById(_selectedMountId, _availableMounts);
  }
  ...
  Future<void> _loadLenses() async {
    final lenses = await _lensRepository.getLenses();
    if (!mounted) {
      return;
    }
  ...
  void _applyLensToExtension(Lens lens) {
    ...
    setState(() {
      ...
      _extensionFocalMm.text = _controller.formatLensFocal(focal);
  ...
  void _applyLensToReverse(Lens lens) {
    ...
    setState(() {
      ...
      _reverseFocalMm.text = _controller.formatLensFocal(focal);
  ...
  void _applyLensToDualTaking(Lens lens) {
    ...
    setState(() {
      ...
      _dualTakingFocalMm.text = _controller.formatLensFocal(focal);
  ...
  void _applyLensToDualFront(Lens lens) {
    ...
    setState(() {
      ...
      _dualFrontFocalMm.text = _controller.formatLensFocal(focal);
    });
    _calculateDualLens();
  }
  ...
  void _calculateExtension() {
    final state = _controller.calculateExtensionFromForm(
      focalText: _extensionFocalMm.text,
      apertureText: _extensionAperture.text,
      minimumFocusDistanceText: _extensionMinFocusM.text,
      extensionLengthText: _extensionTubeMm.text,
      sensor: _selectedSensor,
    );
    setState(() {
      switch (state) {
        case MacroCalculationSuccess<ExtensionTubeResult>(:final result):
          _extensionError = null;
          _extensionResult = result;
        case MacroCalculationError<ExtensionTubeResult>(:final message):
          _extensionError = message;
  ...
  void _calculateReverse() {
    final state = _controller.calculateReverseFromForm(
      focalText: _reverseFocalMm.text,
      apertureText: _reverseAperture.text,
      extraExtensionText: _reverseExtraExtensionMm.text,
      mount: _selectedMount,
      sensor: _selectedSensor,
    );
    setState(() {
      switch (state) {
        case MacroCalculationSuccess<ReverseLensResult>(:final result):
          _reverseError = null;
          _reverseResult = result;
        case MacroCalculationError<ReverseLensResult>(:final message):
          _reverseError = message;
          _reverseResult = null;
      }
    });
  }
  ...
  void _calculateDualLens() {
    final state = _controller.calculateDualFromForm(
      takingFocalText: _dualTakingFocalMm.text,
      takingApertureText: _dualTakingAperture.text,
      frontFocalText: _dualFrontFocalMm.text,
      sensor: _selectedSensor,
    );
    setState(() {
      switch (state) {
        case MacroCalculationSuccess<DualLensMacroResult>(:final result):
          _dualError = null;
          _dualResult = result;
        case MacroCalculationError<DualLensMacroResult>(:final message):
          _dualError = message;
          _dualResult = null;
      }
    });
      ...
      MaterialPageRoute(
        builder: (_) => FocusStackingPlannerScreen(
          ...
          initialPreset: FocusStackingPreset.extension(
            ...
            subjectDepthM: _controller.suggestMacroSubjectDepth(
