...
- Flutter baseline moved from 3.44.x to 3.47.5 in CI, the documentation, and the sandbox wrapper, with
  the two committed goldens regenerated by the new SDK in the same change (the 3.41/3.44 captures differed
  by 0.02% and 0.01% of pixels). `pubspec.lock` gained the SDK-pinned dev dependencies (matcher 0.12.20,
  meta 1.19.0, test_api 0.7.12, vector_math 2.4.3) and `pubspec.yaml` keeps its `>=3.41.0` floor.
...
Verification: 434 local tests (this pass added the gates that closed the remaining 200% gaps - every
...
labels, and four of the widest results re-rendered on a 320x568 phone, the narrowest supported width),
clean analyzer and formatter, all eight host integration journeys green, and a Linux release build.
