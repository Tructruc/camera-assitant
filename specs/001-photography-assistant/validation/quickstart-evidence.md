...
(340 → 434 tests). Rows 21 and 28 are the only ones with device-only parts, and the sections after the
table list the rest.

Commands and results:

```sh
./.tooling/flutterw --no-version-check test --no-pub --concurrency=1   # 434 passed
